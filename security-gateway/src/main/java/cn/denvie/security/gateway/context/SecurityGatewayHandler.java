...
import cn.denvie.security.gateway.service.*;
...
@Component
@Slf4j
public class SecurityGatewayHandler implements InitializingBean, ApplicationContextAware {
    ...
    private SecurityGatewayRegisterCenter registerCenter;

    public SecurityGatewayHandler() {
        parameterNameDiscoverer = new DefaultParameterNameDiscoverer();
    }

    @Override
    public void setApplicationContext(ApplicationContext context) throws BeansException {
        registerCenter = new SecurityGatewayRegisterCenter(context);
    }

    @Override
    public void afterPropertiesSet() throws Exception {
        registerCenter.loadApiFromSpringBeans();
    }
    ...
    public void handle(HttpServletRequest request, HttpServletResponse response, String httpMethod) {
        ...
        SecurityGatewayRegisterCenter.ApiRunnable apiRunnable;
        ...
        try {
            ...
            if (!StringUtils.isEmpty(apiRequest.getAccessToken())) {
                apiToken = checkToken(apiRequest, apiRunnable.getSecurityApi().needLogin());
            ...
            // 登录验证
            if (apiRunnable.getSecurityApi().needLogin()) {
    ...
    public void handleSub(HttpServletRequest request, HttpServletResponse response, String httpMethod) {
        ...
        try {
            ...
            apiRequest = buildApiRequest(originalApiParam);
            // 注入密钥
            apiRequest.setSecret(securityGatewayProperties.getSubSecret());
            apiRequest.setPrivateScret(securityGatewayProperties.getSubPrivateSecret());
            // 验证签名和参数
    ...
    private ApiResponse doInvokeApi(HttpServletRequest request, HttpServletResponse response, String httpMethod,
                                    ApiParam originalApiParam, SecurityGatewayRegisterCenter.ApiRunnable apiRunnable, ApiRequest apiRequest) throws Exception {
        Object[] args = buildParams(apiRunnable, apiRequest.getParams(), request, response, apiRequest);
        if (securityGatewayProperties.isEnableLogging()) {
    ...
    private ApiParam resolveApiParam(HttpServletRequest request) {
        ...
        if (securityGatewayProperties.getParamType() == ParamType.BODY) {
    ...
    private ApiRequest checkSignAndParams(ApiRequest apiRequest, ApiToken apiToken) throws ApiException {
        ...
        if (securityGatewayProperties.isCheckTimestamp() && diffTime > securityGatewayProperties.getTimestampDiffer()) {
            throw new ApiException(ApiCode.CHECK_ILLEGAL_TIME);
        }

        // 客户端设备校验
        if (securityGatewayProperties.isCheckDevice()) {
    ...
    private void decryptParams(ApiRequest apiRequest) throws ApiException {
        try {
            if (securityGatewayProperties.getEncryptType() == EncryptType.AES) {
                ...
                apiRequest.setParams(temp);
            } else if (securityGatewayProperties.getEncryptType() == EncryptType.BASE64) {
                String temp = apiRequest.getParams();
    ...
    private SecurityGatewayRegisterCenter.ApiRunnable validateSysParams(ApiParam originalApiParam) throws ApiException {
        SecurityGatewayRegisterCenter.ApiRunnable api;
        ...
        } else if ((api = registerCenter.findApiRunnable(originalApiParam.getName())) == null) {
            throw new ApiException(ApiCode.API_UN_EXIST);
        }
        return api;
    }

    private SecurityGatewayRegisterCenter.ApiRunnable validateSubParams(ApiParam subParam) throws ApiException {
        SecurityGatewayRegisterCenter.ApiRunnable apiSub;
        if (StringUtils.isEmpty(subParam.getName())) {
            throw new ApiException(ApiCode.API_NAME_NULL);
        } else if (StringUtils.isEmpty(subParam.getSign())) {
            throw new ApiException(ApiCode.API_SIGN_NULL);
        } else if ((apiSub = registerCenter.findApiRunnable(subParam.getName())) == null) {
            throw new ApiException(ApiCode.API_UN_EXIST);
        }
        // @ApiMapping中的needLogin设置为false的才能通过subApi调用
        if (apiSub.getSecurityApi().needLogin()) {
     ...
     * 验证业务参数，和构建业务参数对象
     */
    private Object[] buildParams(SecurityGatewayRegisterCenter.ApiRunnable apiRunnable, String paramJson, HttpServletRequest request,
        ...
        String[] paramNames = apiRunnable.getSecurityApi().paramNames();
        if (securityGatewayProperties.isEnableLogging()) {
        ...
        for (int i = 0; i < paramTypes.length; i++) {
            ...
            } else if ((resolver = registerCenter.supportsParameter(apiRunnable.getMethodParameters().get(i))) != null) {
                try {
