 ...
 */
@Slf4j
public class SecurityGatewayRegisterCenter {
    private final ApplicationContext applicationContext;
    private final ConcurrentHashMap<String, ApiRunnable> apiMap = new ConcurrentHashMap();
    private final List<HandlerMethodArgumentResolver> methodArgumentResolvers = new ArrayList<>();

    // spring ioc
    public SecurityGatewayRegisterCenter(ApplicationContext applicationContext) {
    ...
    public void loadApiFromSpringBeans() {
        ...
        for (String beanName : beanNames) {
            ...
            for (Method m : type.getDeclaredMethods()) {
                ...
                SecurityApi securityApi = AnnotationUtils.findAnnotation(m, SecurityApi.class);
                if (securityApi != null) {
                    addApiItem(securityApi, beanName, m, type);
                }
            }
        }
    }
    ...
    private void addApiItem(SecurityApi securityApi, String beanName, Method method, Class<?> type) {
        ApiRunnable apiRunnable = new ApiRunnable();
        apiRunnable.apiName = securityApi.value();
        apiRunnable.targetName = beanName;
        apiRunnable.targetMethod = method;
        apiRunnable.targetMethodName = type.getName() + "." + method.getName();
        apiRunnable.securityApi = securityApi;
        // MethodParameter
