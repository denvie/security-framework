...
public class RSAUtils {
    ...
    public static Map<String, String> generateRSAKeyBase64(int keySize) throws NoSuchAlgorithmException {
        ...
        if (byteMap != null && byteMap.size() == 2) {
            stringMap.put(KEY_PRIVATE, Base64.encodeBase64String(byteMap.get(KEY_PRIVATE)));
            stringMap.put(KEY_PUBLIC, Base64.encodeBase64String(byteMap.get(KEY_PUBLIC)));
        }
        return stringMap;
    }
    ...
    public static String encryptByPublicKey(String rsaPublicKeyBase64, String src) throws Exception {
        byte[] bytes = encryptByPublicKey(Base64.decodeBase64(rsaPublicKeyBase64.getBytes(CHARSET)), src);
        if (bytes != null) {
    ...
    public static String decryptByPrivateKey(String rsaPrivateKeyBase64, String srcBase64) throws Exception {
        byte[] bytes = decryptByPrivateKey(Base64.decodeBase64(rsaPrivateKeyBase64.getBytes(CHARSET)),
                Base64.decodeBase64(srcBase64.getBytes(CHARSET)));
        if (bytes != null) {
            try {
    ...
    public static String encryptByPrivateKey(String rsaPrivateKeyBase64, String src) throws Exception {
        byte[] bytes = encryptByPrivateKey(Base64.decodeBase64(rsaPrivateKeyBase64.getBytes(CHARSET)), src);
        if (bytes != null) {
            return Base64.encodeBase64String(bytes);
        }
        return null;
    ...
    public static String decryptByPublicKey(String rsaPublicKeyBase64, String srcBase64) throws Exception {
        byte[] bytes = decryptByPublicKey(
                Base64.decodeBase64(rsaPublicKeyBase64.getBytes(CHARSET)), Base64.decodeBase64(srcBase64));
        if (bytes != null) {
            try {
                return new String(bytes, CHARSET);
            } catch (UnsupportedEncodingException e) {
    ...
    public static String sign(String content, String rsaPrivateKeyBase64) throws Exception {
        byte[] bytes = sign(content, Base64.decodeBase64(rsaPrivateKeyBase64.getBytes(CHARSET)));
        if (bytes != null) {
            return Base64.encodeBase64String(bytes);
        }
        return null;
    ...
    public static boolean signValidate(String src, String signBase64, String rsaPublicKeyBase64) throws Exception {
        return signValidate(src, Base64.decodeBase64(signBase64.getBytes(CHARSET)),
                Base64.decodeBase64(rsaPublicKeyBase64.getBytes(CHARSET)));
    }
    ...
    private static void publicEncodePrivateDecode(String src) throws Exception {
        ...
        // 生成公私钥对
        Map<String, byte[]> keyMap = generateKeyMap();
    ...
    private static void privateSignPublicValidate(String src) throws Exception {
        ...
        System.out.println("私钥签名: " + Base64.encodeBase64String(result));
        // 验签
        System.out.println("公钥验签: " + signValidate(src, result, rsaPublicKeyBytes));
    }

    private static Map<String, byte[]> generateKeyMap() throws NoSuchAlgorithmException {
        Map<String, byte[]> keyMap = generateRSAKey(512);
        System.out.println("RSAPrivateKey: " + Base64.encodeBase64String(keyMap.get(KEY_PRIVATE)));
        System.out.println("RSAPublicKey:  " + Base64.encodeBase64String(keyMap.get(KEY_PUBLIC)));
        return keyMap;
    }
}
